...
# ve bu davranış öbeklenmelerine göre gruplar oluşturulacaktır.
...
# 3. Omnichannel müşterilerin hem online hem de offline platformlardan
# alışveriş yaptığını ifade etmektedir.
# Herbir müşterinin toplam alışveriş sayısı ve harcaması için
...
# qcut() -> değerleri sıralar, istediğimiz sayıda parçaya böler, istediğimiz şekilde etiketler
# rank(first) - farklı aralıklara aynı değerlerin gelmesi sorununu çözer
...
# 3. recency_score ve frequency_score ve monetary_score'un
# tek bir değişken olarak ifade edilmesi ve RFM_SCORE olarak kaydedilmesi
...
# metin içindeki belirli örüntüleri tanımlamak ve eşleştirmek için kullanılır
# ".... görünce ... yaz" şeklinde davranmasını sağlar
...
# müşterilerle özel olarak iletişime geçilmek isteniliyor.
...
# olarak kaydediniz.
target_segments_customer_ids_ = rfm[rfm["segment"].isin(["cant_loose","hibernating","new_customers"])]["customer_id"]
cust_ids_ = df[(df["master_id"].isin(target_segments_customer_ids)) & ((df["interested_in_categories_12"].str.contains("ERKEK"))|(df["interested_in_categories_12"].str.contains("COCUK")))]["master_id"]
cust_ids.shape
